...
loo_survextrap <- function(x, standata, loglik_fn){
  ...
  if (any(is.nan(exp(ll)))){
    warning("NaN produced for likelihood when computing LOO statistic")
    res <- NA
  }
  else {
...
loglik_external <- function(x, standata){
  ...
  lp_stop <- lp_stop[,inds,drop=FALSE] # niter rows x ni cols
  lp_start <- lp_start[,inds,drop=FALSE]
  y <- matrix(y, nrow=1)[rep(1,niter),,drop=FALSE] # niter x ni
